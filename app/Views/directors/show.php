...
<div class="container mx-auto px-4 py-10">
    ...
    <div class="max-w-5xl mx-auto mb-6">
        <nav class="flex items-center text-xs text-slate-500 space-x-2">
            <a href="<?= $baseUrl ?>/" class="hover:text-emerald-night transition">হোম</a>
            <span>/</span>
            <a href="<?= $baseUrl ?>/directors" class="hover:text-emerald-night transition">জেলা পরিচালকবৃন্দ</a>
            <span>/</span>
            <span class="text-emerald-night font-bold truncate"><?= htmlspecialchars($director['name']) ?></span>
        </nav>
    </div>
    ...
    <div class="max-w-5xl mx-auto bg-[#fffefb] rounded-3xl shadow-sm border border-[#e5ddcb] overflow-hidden mb-10">
            ...
            <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6 text-center md:text-left">
                ...
                <div class="relative shrink-0">
                    <img src="<?= htmlspecialchars($avatarUrl) ?>" 
                         ...
                         loading="lazy"
                         title="ডেমো ছবি সাইজ: ৪০০x৪০০ পিক্সেল (রেসপন্সিভ অবজেক্ট-ফিট)"
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-2">
                        ...
                        <?php if ($director['status'] === 'active'): ?>
                            <span class="px-3 py-1 bg-emerald-700/80 text-emerald-100 text-xs font-bold rounded-full border border-emerald-500 flex items-center">
                                <span class="w-2 h-2 rounded-full bg-emerald-300 inline-block mr-1.5 animate-pulse"></span> দায়িত্বপালনরত
                            </span>
                ...
                <div class="flex flex-col gap-2.5 w-full md:w-auto">
                    <a href="tel:<?= preg_replace('/[^0-9+]/', '', $director['phone']) ?>" 
                        class="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition flex items-center justify-center">
                    ...
                    <?php if (!empty($director['whatsapp'])): ?>
                        <a href="https://wa.me/88<?= preg_replace('/[^0-9]/', '', $director['whatsapp']) ?>" target="_blank"
                            class="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition flex items-center justify-center">
        ...
        <div class="p-6 md:p-8 bg-slate-50 border-b border-slate-200">
            <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider mb-2 flex items-center">
                <svg class="w-4 h-4 text-emerald-800 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <?php if (!empty($director['address'])): ?>
                <div class="mt-4 pt-3 border-t border-slate-200 flex items-center text-xs text-slate-600">
                    <svg class="w-4 h-4 text-emerald-800 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    <span><strong>যোগাযোগের প্রাতিষ্ঠানিক ঠিকানা:</strong> <?= htmlspecialchars($director['address']) ?></span>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="max-w-5xl mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-slate-200 mb-6">
            <div>
                <span class="text-xs font-bold text-amber-700 uppercase tracking-wider">কুরআন মুয়াল্লিম নেটওয়ার্ক</span>
            ...
            <div class="mt-3 sm:mt-0">
                <a href="<?= $baseUrl ?>/admission" class="bg-emerald-night hover:bg-emerald-deep text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow transition inline-flex items-center">
                    <svg class="w-4 h-4 mr-1.5 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                    </svg>
                    শিক্ষক নিয়োগ বা প্রশিক্ষণে আবেদন
                </a>
            </div>
        </div>
        ...
        <?php else: ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <?php foreach ($teachers as $idx => $t): ?>
                    <div class="bg-[#fffefb] p-5 rounded-2xl border border-[#e5ddcb] hover:border-gold-rich/80 shadow-sm hover:shadow-md transition">
                        <div class="flex items-start space-x-3">
                            <div class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-950 flex items-center justify-center font-bold text-base shrink-0 border border-emerald-200">
                                <?= mb_substr($t['name'], 0, 1, 'UTF-8') ?>
                            </div>
                            <div class="overflow-hidden flex-1">
                                <h4 class="font-bold text-slate-900 text-sm truncate"><?= htmlspecialchars($t['name']) ?></h4>
                                <p class="text-xs text-emerald-800 font-medium mt-0.5 flex items-center truncate">
                                    <svg class="w-3.5 h-3.5 text-emerald-600 mr-1 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                            ...
                            <div>
                                <span class="text-amber-800 font-bold"><?= \Core\BengaliHelper::toBengaliNumber($t['total_students']) ?> জন শিক্ষার্থী</span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <div class="mt-10 bg-emerald-50/80 p-6 md:p-8 rounded-3xl border border-emerald-200/90 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
